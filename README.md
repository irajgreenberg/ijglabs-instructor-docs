...
The GitHub Actions workflow in `.github/workflows/deploy.yml` verifies the build on push to `main`, rsyncs the project to cortex-studio, runs `npm ci && npm run build`, reloads or starts PM2, saves the PM2 process list, and checks `http://localhost:3219/`.

Required repository secrets/variables:

- `CORTEX_SSH_KEY`: private SSH key with access to cortex-studio.
- `CORTEX_HOST`: optional, defaults to `cortex-studio`.
